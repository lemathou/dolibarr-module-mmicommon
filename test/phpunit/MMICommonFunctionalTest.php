...
/**
 * \file    test/phpunit/MMICommonFunctionalTest.php
 * \ingroup mmicommon
 ...
 *
 * @package Testmmicommon
 */
class MMICommonFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modMMICommon')->click();
		} else {
			// Disable the module
			$this->byHref('modMMICommon')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/mmicommon/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('mmicommon/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/mmicommon/admin/about.php');
		$this->authenticate();
		return $this->assertContains('mmicommon/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('mmicommonwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modMMICommon_MMICommonTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modMMICommon_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
