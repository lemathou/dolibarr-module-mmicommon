...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/".$module_name."/oc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = $moduleprefix.'_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = $moduleprefix.'_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, $modulecontext);
			...
			if ($val['enabled']==1) {
				...
				if ($val['type'] == 'textarea') {
					...
					if (!empty($conf->global->{$constname})) {
						$strlen = strlen($conf->global->{$constname});
						$straff = $strlen>=10 ?2 :($strlen>=5 ?1 :0);
						print ($straff ?substr($conf->global->{$constname}, 0, 1) :'')
							.str_repeat('*', $strlen-$straff )
							.($straff>=2 ?substr($conf->global->{$constname}, -1, 1) :'');
					}
					else {
						echo '';
					}
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = $moduleprefix.'_'.strtoupper($myTmpObjectKey).'_ADDON';
